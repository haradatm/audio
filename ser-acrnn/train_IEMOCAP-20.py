...
def load_and_mfcc(filename, labels={}):
    X, y = [], []
...
def main():
    ...
    parser.add_argument('--out', '-o', default='iemocap-fbank-20-300_b040_e300_d010_adam', help='directory to output the result')
    ...
    if args.type == 'fbank':
        X_train, y_train, labels = load_and_fbank(args.train, labels=labels)
        ...
        X_train, y_train, labels = load_and_mfcc(args.train, labels=labels)
    ...
    for epoch in range(start_epoch, args.epoch + 1):
        ...
        # training
        train_iter = sorted_batch_iter([(x, t) for x, t in zip(X_train, y_train)], args.batchsize, shuffle=True, random_state=seed)
        sum_train_loss = 0.
        ...
        # evaluation
        valid_iter = batch_iter([(x, t) for x, t in zip(X_valid, y_valid)], args.batchsize, shuffle=False)
        sum_test_loss = 0.
    ...
    test_iter = batch_iter([(x, t) for x, t in zip(X_test, y_test)], args.batchsize, shuffle=False)
